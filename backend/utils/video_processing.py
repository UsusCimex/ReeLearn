...
class SmartVideoFragmenter:
    def __init__(self, min_duration: float = settings.VIDEO_MIN_FRAGMENT_DURATION, max_duration: float = settings.VIDEO_MAX_FRAGMENT_DURATION, optimal_duration: float = settings.VIDEO_OPTIMAL_DURATION, default_language: str = settings.VIDEO_DEFAULT_LANGUAGE, max_sentences: int = settings.VIDEO_MAX_SENTENCES_PER_FRAGMENT):
    ...
    def _adjust_fragment_boundaries(self, segments):
        if not segments:
            return []
        optimized = []
        current = {"segments": [segments[0]], "start": segments[0]["start_time"], "end": segments[0]["end_time"], "language": segments[0]["language"]}
        for seg in segments[1:]:
            ...
            if current_duration < self.optimal_duration and gap <= 0.8 and potential_duration <= self.optimal_duration * 1.2:
                ...
                optimized.append({"start_time": current["start"], "end_time": current["end"], "text": combined_text, "language": current["language"]})
                current = {"segments": [seg], "start": seg["start_time"], "end": seg["end_time"], "language": seg["language"]}
        if current["segments"]:
            combined_text = " ".join(s["text"] for s in current["segments"])
            optimized.append({"start_time": current["start"], "end_time": current["end"], "text": combined_text, "language": current["language"]})
        return optimized
    def process_subtitles(self, subtitles):
        ...
        for seg in optimized:
            ...
            frag = VideoFragment(start_time=seg["start_time"], end_time=seg["end_time"], text=seg["text"], sentences=sentences, language=seg["language"], tags=[], s3_url="", speech_confidence=1.0, no_speech_prob=0.0)
    ...
    def process_video(self, video_path: str):
        ...
        for frag in raw_fragments:
            lang = self._detect_language(frag.text)
            frag.language = lang
            segments.append({"start_time": frag.start_time, "end_time": frag.end_time, "text": frag.text, "language": lang})
